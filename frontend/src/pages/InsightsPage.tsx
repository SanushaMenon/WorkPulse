...
const P = {
  ...
  amber: "#f59e0b", emerald: "#10b981", sky: "#38bdf8",
};
...
function StatCard({ label, value, suffix, delay, color }: { label: string; value: number; suffix?: string; delay: string; color?: string }) {
  ...
  return (
    <Box sx={{
      ...
      "&::before": { content: '""', position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: color ? color : GRAD },
    }}>
      <Typography sx={{ fontSize: "2.2rem", fontWeight: 800, lineHeight: 1, fontFamily: "monospace", background: color ? color : GRAD, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
        {count}{suffix || ""}
      </Typography>
...
function SkillPill({ name, count, idx, color }: { name: string; count: number; idx: number; color: string }) {
  return (
    <Box sx={{
      ...
      <Typography sx={{ flex: 1, fontSize: "12px", color: P.text, fontFamily: "monospace" }}>{name}</Typography>
      <Typography sx={{ fontSize: "11px", color, fontWeight: 700, fontFamily: "monospace" }}>{count}</Typography>
    </Box>
  );
}
...
const InsightsPage: React.FC = () => {
  ...
  const trendData = {
    ...
    datasets: [
      { label: "Positive %", data: sentimentTrend.map(s => s.positive), borderColor: P.pos, backgroundColor: P.pos + "20", fill: true, tension: 0.4, pointRadius: 4, pointHoverRadius: 6 },
      { label: "Neutral %", data: sentimentTrend.map(s => s.neutral), borderColor: P.neu, backgroundColor: P.neu + "10", fill: false, tension: 0.4, pointRadius: 3, pointHoverRadius: 5, borderDash: [5, 5] },
      { label: "Negative %", data: sentimentTrend.map(s => s.negative), borderColor: P.neg, backgroundColor: P.neg + "10", fill: false, tension: 0.4, pointRadius: 3, pointHoverRadius: 5 },
    ],
  };
  const trendOptions = {
    responsive: true, maintainAspectRatio: false,
    plugins: { legend: { labels: { color: P.muted, font: { size: 10 }, usePointStyle: true, pointStyle: "circle" } }, tooltip: tooltipDefaults },
    scales: {
      x: { grid: { display: false }, ticks: { color: P.muted, font: { size: 10 } } },
      y: { grid: { color: "rgba(255,255,255,0.05)" }, ticks: { color: P.muted, font: { size: 10 }, callback: (v: any) => v + "%" }, min: 0, max: 100 },
    },
  };
  ...
  const deptBarData = {
    labels: deptLabels,
    datasets: [
      { label: "Positive", data: deptLabels.map(d => deptSentiment[d]?.positive || 0), backgroundColor: P.pos + "cc", borderRadius: 4, borderSkipped: false as const },
      { label: "Neutral", data: deptLabels.map(d => deptSentiment[d]?.neutral || 0), backgroundColor: P.neu + "cc", borderRadius: 4, borderSkipped: false as const },
      { label: "Negative", data: deptLabels.map(d => deptSentiment[d]?.negative || 0), backgroundColor: P.neg + "cc", borderRadius: 4, borderSkipped: false as const },
    ],
  };
  const deptBarOptions = {
    ...
    plugins: { legend: { labels: { color: P.muted, font: { size: 10 }, usePointStyle: true, pointStyle: "circle" } }, tooltip: tooltipDefaults },
    scales: {
      x: { stacked: true, grid: { display: false }, ticks: { color: P.muted, font: { size: 10 } } },
      y: { stacked: true, grid: { color: "rgba(255,255,255,0.05)" }, ticks: { color: P.muted, font: { size: 10 } }, beginAtZero: true },
    },
  };
      ...
      <Box sx={{ color: P.text }}>
        <Box sx={{ mb: "36px", animation: "slideUp 0.4s ease both" }}>
          ...
          <Typography sx={{ color: P.muted, fontSize: "13px", fontFamily: "monospace" }}>
            Organization-wide analytics, department health, risk alerts, and skill gap analysis.
          </Typography>
        </Box>
        ...
        {data && !loading && (
          <Grid container spacing={2.5}>
            ...
            <Grid item xs={6} sm={4} md={2.4}>
              <StatCard label="Total Reviews" value={totCount} delay="0s" />
            </Grid>
            <Grid item xs={6} sm={4} md={2.4}>
              <StatCard label="This Month" value={data?.thisMonth ?? 0} delay="0.07s" color={P.sky} />
            </Grid>
            <Grid item xs={6} sm={4} md={2.4}>
              <StatCard label="Positive %" value={posPercent} suffix="%" delay="0.14s" color={P.emerald} />
            </Grid>
            <Grid item xs={6} sm={4} md={2.4}>
              <StatCard label="Avg Rating" value={Math.round(avgRating * 10) / 10} suffix="/5" delay="0.21s" color={P.amber} />
            </Grid>
            <Grid item xs={6} sm={4} md={2.4}>
              <StatCard label="High Priority" value={highPriority} delay="0.28s" color={P.neg} />
            </Grid>
              ...
              <Panel title="Sentiment Trend">
                <Box sx={{ height: 240 }}>
                  {sentimentTrend.length > 0
                    ...
                    : <Typography sx={{ color: P.muted, fontSize: "13px", fontFamily: "monospace" }}>No trend data available yet.</Typography>
                  }
                </Box>
              </Panel>
            </Grid>
              ...
              <Panel title="Department Sentiment" delay="0.2s">
                <Box sx={{ height: 250 }}>
                  {deptLabels.length > 0
                    ...
                    : <Typography sx={{ color: P.muted, fontSize: "13px", fontFamily: "monospace" }}>No department data yet.</Typography>
                  }
                </Box>
              </Panel>
            </Grid>

            {/* Sentiment Volume Bar */}
            <Grid item xs={12} md={6}>
            ...
            <Grid item xs={12} md={4}>
              <Panel title="Top Strengths (Org-wide)" delay="0.3s">
                {topStrengths.length === 0
                  ? <Typography sx={{ color: P.muted, fontSize: "13px", fontFamily: "monospace" }}>No strengths data yet.</Typography>
                  : <Stack spacing={1}>
                    {topStrengths.map(([name, count], i) => <SkillPill key={name} name={name} count={count} idx={i} color={P.emerald} />)}
                  </Stack>
                }
              </Panel>
            </Grid>
            ...
            <Grid item xs={12} md={4}>
              <Panel title="Top Improvements (Org-wide)" delay="0.3s">
                {topImprovements.length === 0
                  ? <Typography sx={{ color: P.muted, fontSize: "13px", fontFamily: "monospace" }}>No improvements data yet.</Typography>
                  : <Stack spacing={1}>
                    {topImprovements.map(([name, count], i) => <SkillPill key={name} name={name} count={count} idx={i} color={P.amber} />)}
                  </Stack>
                }
              </Panel>
            </Grid>
            ...
            {/* Employee Reviews */}
            <Grid item xs={12}>
              <Panel title={`Employee Reviews (${filteredReviews.length})`} delay="0.4s">
                <Box
                  component="input"
